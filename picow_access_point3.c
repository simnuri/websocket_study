...
const size_t MASK_BIT_MASK = 1 << 7;
const size_t PRE_LEN_MASK = ~MASK_BIT_MASK;

const size_t PRE_LEN_MAX = 125;
const size_t PRE_LEN_EX2 = 126;
const size_t PRE_LEN_EX8 = 127;


const size_t MIN_HEADER_LEN = 2;
const size_t MAX_HEADER_LEN = 14;

bool isHeaderComplete(struct tcp_pcb* pcb, struct pbuf* pb, TCP_CLIENT_T* client_conn) {

    size_t bytes_needed = MIN_HEADER_LEN;

    if(client_conn->header_bytes < bytes_needed) {
        return false;
    }

    uint8_t pre_len = getPreLen(client_conn->header);
    if(pre_len == PRE_LEN_EX2) bytes_needed += 2;

    if(pre_len == PRE_LEN_EX8) bytes_needed += 8;

    if(client_conn->header[1]&MASK_BIT_MASK) bytes_needed += 4;

    return client_conn->header_bytes >= bytes_needed;
}

bool message_builder_process(struct tcp_pcb* pcb, struct pbuf* pb, TCP_CLIENT_T* client_conn, uint8_t byte) {

    bool consumed = false;
    if(!isHeaderComplete(pcb, pb, client_conn)) {
        client_conn->header[client_conn->header_bytes++] = byte;
        consumed = true;
    }
}
...
bool ws_handler_process(struct tcp_pcb* pcb, struct pbuf* pb, TCP_CLIENT_T* client_conn) {
    ...
    for(i = 0; i < pb->tot_len; i++) {
        pbuf_get_at(pb, i);
        if(!message_builder_process(pcb, pb, client_conn, c)) {
            return false;
        }
    }

    if(client_conn->header[1]&MASK_BIT_MASK) return false; // Frames from client must be masked
